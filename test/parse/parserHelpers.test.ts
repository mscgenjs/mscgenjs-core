...
describe("parserHelpers.nameValue2Option", () => {
  it("Name and value return a name/ value object", () => {
    deepEqual(parserHelpers.nameValue2Option("thisisaname", "thisisavalue"), {
...
describe("parserHelpers.entityExists", () => {
  ...
  it("returns false if no entity exists with the given name", () => {
    deepEqual(
      parserHelpers.entityExists(lEntitiesFixture, "SOAP layer"),
      false,
    );
    ...
    doesNotThrow(() =>
      parserHelpers.checkForUndeclaredEntities(
        ...
        [
          [
            {
              from: "this entity exists",
              kind: "=>",
              to: "this entity exists too",
            },
          ],
        ],
      ),
...
describe("parserHelpers.getMetaInfo", () => {
  const lNoExtendedFeatures = {
    extendedArcTypes: false,
    extendedFeatures: false,
    extendedOptions: false,
  };
  const lExtendedOptions = {
    extendedArcTypes: false,
    extendedFeatures: true,
    extendedOptions: true,
  };
  const lExtendedArcTypes = {
  ...
  it("boundary values deliver meta info with everything on false", () => {
    ...
    deepEqual(
      parserHelpers.getMetaInfo(undefined, undefined),
      lNoExtendedFeatures,
    );
    deepEqual(parserHelpers.getMetaInfo({}, []), lNoExtendedFeatures);
  });
  ...
  it("standard mscgen options deliver meta info with everything on false", () => {
    deepEqual(
      parserHelpers.getMetaInfo({ wordwraparcs: true }),
      lNoExtendedFeatures,
    );
  ...
  it("no options but regular arcs deliver meta info with everything on false", () => {
    deepEqual(
      parserHelpers.getMetaInfo(
        {},
        require("../fixtures/astemptylinesinboxes.json").arcs,
      ),
      lNoExtendedFeatures,
    );
    deepEqual(
      parserHelpers.getMetaInfo(
        {},
        require("../fixtures/test01_all_possible_arcs_mscgen.json").arcs,
      ),
      lNoExtendedFeatures,
    );
  });
  ...
  it("extended options deliver meta info with options & features on true", () => {
    deepEqual(
      parserHelpers.getMetaInfo({ watermark: "extended!!" }),
      lExtendedOptions,
    );
  ...
  it("no options but extended arc types deliver meta info arctypes & features on true", () => {
    deepEqual(
      parserHelpers.getMetaInfo(
        {},
        require("../fixtures/bumpingboxes.json").arcs,
      ),
      lExtendedArcTypes,
    );
    deepEqual(
      parserHelpers.getMetaInfo(
        {},
        require("../fixtures/test01_all_possible_arcs.json").arcs,
      ),
      lExtendedArcTypes,
    );
  });
