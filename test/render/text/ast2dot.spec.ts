...
describe("render/text/ast2dot", () => {
  ...
  describe("explodeBroadcasts", () => {
    test("leave asts without broadcasts alone", () => {
      expect(explodeBroadcasts(fix.astAltWithinLoop)).toEqual(
        fix.astAltWithinLoop
      );
    });
    test("explode b->* to parallel calls to all other entities", () => {
      expect(explodeBroadcasts(fix.astSimpleBroadcast)).toEqual(
        fix.astSimpleBroadcastExploded
      );
    });
    test("explode a little more complex broadcast ast to parallel calls to all other entities", () => {
      expect(explodeBroadcasts(fix.astComplexerBroadcast)).toEqual(
        fix.astComplexerBroadcastExploded
      );
    });
    test("correctly explode a broadcast that has other arcs in the same arc row", () => {
      expect(explodeBroadcasts(fix.astSameArcRowBroadcast)).toEqual(
