        ...
        lRetVal = lOptions
            .reduce(
                (pPrevOption, pCurOption) =>
                    pPrevOption + gConfig.renderOptionfn(pCurOption) + gConfig.option.separator,
...
function renderEntities(pEntities) {
    ...
    if (pEntities.length > 0) {
        lRetVal = pEntities
            .slice(0, -1)
            .reduce(
                (pPrev, pEntity) => pPrev + renderEntity(pEntity) + gConfig.entity.separator,
        ...
        lRetVal = lAttributes
            .reduce(
                (pPreviousAttribute, pCurrentAttribute) =>
                    pPreviousAttribute + gConfig.renderAttributefn(pCurrentAttribute) + gConfig.attribute.separator,
...
function renderArcLine(pArcLine, pIndent) {
    ...
    if (pArcLine.length > 0) {
        lRetVal = pArcLine
            .slice(0, -1)
            .reduce(
                (pPrev, pArc) => pPrev + pIndent + renderArc(pArc, pIndent) + gConfig.arcline.separator,
